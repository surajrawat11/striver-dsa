...
2. Rearrange elements so that those smaller than pivot
   are on the left, and those greater are on the right.
3. Recursively apply Quick Sort to left and right subarrays.

Properties:
- Average Time Complexity: O(n log n)
- Worst Case Time Complexity: O(n^2) (when pivot is poorly chosen)
- Space Complexity: O(log n) due to recursion stack
- Not a stable sort
- Very efficient in practice for large datasets
*/
